...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full px-4 py-4 z-[100] bg-white shadow-sm border-b border-slate-100">
      <div className="mx-auto max-w-7xl flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center space-x-8">
          {['Home', 'Packages', 'Blog', 'About'].map((item) => (
            <li key={item}>
              <Link 
                href={item === 'Home' ? '/' : `/${item.toLowerCase()}${item === 'Packages' ? 'List' : ''}`}
          ...
          {isLoggedIn ? (
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-3 text-slate-700 hover:text-orange-500 transition-colors group"
              >
                <div className="w-9 h-9 rounded-xl overflow-hidden border-2 border-slate-50 shadow-sm group-hover:border-orange-200 transition-all relative bg-slate-100">
                  ...
                  <ul className="space-y-1 px-2">
                    <li>
                      <Link 
                        ...
                        className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-orange-500 rounded-xl transition-all"
                      >
                        My Profile
                      </Link>
                    </li>
                    ...
                    <li className="pt-1 border-t border-slate-50 mt-1">
                      <button
                        onClick={handleLogout}
                        className="w-full text-left px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 rounded-xl transition-all"
                      >
                        Sign out
                      </button>
                    </li>
            ...
            <Link
              href="/login"
              className="bg-orange-500 hover:bg-orange-600 px-8 py-2.5 rounded-lg font-bold text-white shadow-md hover:shadow-orange-100 transition-all active:scale-95"
            >
              Sign in
            </Link>
          )}
